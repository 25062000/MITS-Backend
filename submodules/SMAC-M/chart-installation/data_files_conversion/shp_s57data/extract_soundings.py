...
# Set the axis mapping strategy to revert to the old behavior in GDAL 3
ogr.UseExceptions()  # Enable exceptions
...
def convert_dataset(
    ...
    file = ogr.Open(str(path),1)
    layer = file.GetLayer()
    
    # Create the new field if it doesn't exist
    field_name = 'DEPTH'
    if layer.GetLayerDefn().GetFieldIndex(field_name) == -1:
    ...
    #     print(f"{i + 1}. {field_defn.GetName()} ({ogr.GetFieldTypeName(field_defn.GetType())})")
    ...
    for feature in layer:
        ...
        feature.SetField('DEPTH', feature.GetField('VALSOU'))
