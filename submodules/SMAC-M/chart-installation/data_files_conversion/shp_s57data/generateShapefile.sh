...
total_files=$(wc -l < "$TMPPATH/FILELIST")
...
do
    ...
    echo "Processing file $current_file_count out of $total_files" >> /tmp/ENC_STATUS
    ...
    do
        ...
        then
            ...
            echoon
            ogr2ogr -append -skipfailures -f "ESRI Shapefile" $output_shp $_FILE --config OGR_S57_OPTIONS "RETURN_PRIMITIVES=ON,RETURN_LINKAGES=ON,LNAM_REFS=ON,SPLIT_MULTIPOINT=ON,ADD_SOUNDG_DEPTH=ON,LIST_AS_STRING=ON" -sql "SELECT '$fileNoExt' AS filename, * FROM $name $where" >> /tmp/errors 2>&1 
            { echooff; } 2>/dev/null

            # ogrtindex ${_FILE}"/index.shp" $_FILE -accept_different_schemas
